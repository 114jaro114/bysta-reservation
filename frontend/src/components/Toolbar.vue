...
<div>
  ...
  || $root.toolbar.fewNewestMessages.overlayNewestMessages == true || $root.toolbar.fewNewestNotifications.overlayNewestNotifications == true || $root.toolbar.fewOldNotifications.overlayOldNotifications == true">
    <!-- ///// -->
    <v-skeleton-loader class="mr-2 hidden-sm-and-down" height="24" width="24" style="margin:12px" type="button" rouned></v-skeleton-loader>
    <v-skeleton-loader class="mr-2 hidden-md-and-up" height="24" width="24" style="margin:12px" type="button" rouned></v-skeleton-loader>

    <v-skeleton-loader class="search ml-3 mr-2" type="chip" height="44"></v-skeleton-loader>
    <v-spacer />
  ...
  <v-app-bar class="toolbar" clipped-left clipped-right dark app color="primary" elevation="0" :elevate-on-scroll="true" v-else>
    ...
    <v-btn class="mr-2" icon @click="openNavigationDrawer">
            ...
            <v-list-item-group color="primary">
                ...
                <v-list-item :key="item.id" :to="'/profile/' + item.id">
                  <v-badge bordered bottom color="grey accent-4" offset-x="25" dot offset-y="18" v-if="item.icon == null">
                    <v-list-item-avatar style="border: 1px; border-color:primary" color="primary" size="36" class="ml-0">
                      <span v-if="item.name != null" class="white--text text-center m-auto">{{item.name.charAt(0).toUpperCase() }}</span>
                    </v-list-item-avatar>
                  </v-badge>
              ...
              <template v-for="item in filteredUsers">
                <v-list-item :key="item.id" :to="'/profile/' + item.id">
                  <v-badge bordered bottom color="grey accent-4" offset-x="25" dot offset-y="18" v-if="item.icon == null">
                    <v-list-item-avatar style="border: 1px; border-color:primary" color="primary" size="36" class="ml-0">
                      <span class="white--text text-center m-auto">{{ item.name.charAt(0).toUpperCase() }}</span>
                    </v-list-item-avatar>
                  </v-badge>
              ...
              <v-dialog v-if="filteredUsers.length != 0 || filteredFriendRequests.length != 0" v-model="dialogFindFriends" fullscreen hide-overlay transition="dialog-bottom-transition">
                ...
                <!-- dialog -->
                <v-card class="toolbar-friends-dialog overflow-hidden">
                  <v-app-bar elevation="0" dark color="primary" elevate-on-scroll>
                    ...
                    <v-list-item :key="item.id + '-newNotf'" @click="selectDataAboutNotification(item)">
                      <v-badge bottom dot bordered :color="getColor(item.status)" offset-x="10" offset-y="10" class="mr-2" v-if="item.name == 'admin'">
                        <v-avatar color="primary" size="40">
                          <span class="md-title font-weight-bold" v-if="$vuetify.theme.dark">
                            <v-img class="ma-lg-0 ma-md-0 ma-auto" :lazy-src="require('../assets/logo/logo_home_theme_dark.png')" max-height="35" max-width="30" :src="require('../assets/logo/logo_home_theme_dark.png')"></v-img>
                          </span>
                          <span class="md-title font-weight-bold" v-else>
                            <v-img class="ma-lg-0 ma-md-0 ma-auto" :lazy-src="require('../assets/logo/logo_home_theme_light.png')" max-height="35" max-width="30" :src="require('../assets/logo/logo_home_theme_light.png')"></v-img>
                          </span>
                        </v-avatar>
                      </v-badge>
                      ...
                      <v-list-item-content>
                        <v-card class="p-2 rounded-lg card-comments" elevation="0">
                          <v-list-item-title class="d-flex justify-start text-subtitle-2 font-weight-bold p-0 pl-1" v-if="item.name == 'admin'">
                            <v-chip small>
                              <v-icon left class="mr-1" small>mdi-account-circle-outline</v-icon>Chata Byšta
                            </v-chip>
                      ...
                      <v-badge bottom dot bordered :color="getColor(item.status)" offset-x="10" offset-y="10" class="mr-2" v-if="item.name == 'admin'">
                        <v-avatar color="primary" size="40">
                          <span class="md-title font-weight-bold">
                            <v-img class="ma-lg-0 ma-md-0 ma-auto" :lazy-src="require('../assets/logo/logo_admin.png')" max-height="30" max-width="30" :src="require('../assets/logo/logo_admin.png')"></v-img>
                          </span>
                        </v-avatar>
                      </v-badge>
                      <v-badge bottom dot bordered :color="getColor(item.status)" offset-x="10" offset-y="10" class="mr-2" v-else>
          ...
          <v-list-item-content class="justify-center p-0">
            <v-list class="p-0">
              <v-list-item-group v-model="$root.language.selectedLang" color="primary">
                <v-list-item class="p-0" v-for="(item, i) in $root.language.countries" :key="i" @click="setlang(item)" :disabled="i == $root.language.selectedLang">
...
import FriendsFriendshipRequests from "./Friends_FriendshipRequests.vue";
...
export default {
  ...
  methods: {
    ...
    openNavigationDrawer() {
      ...
      this.$emit('navigationDrawer', this.openDrawer);
    },
...
.toolbar .search .v-skeleton-loader__chip {
  border-radius: 100px !important;
  width: 187.69px !important;
  height: 44px !important;
}

.toolbar .v-skeleton-loader__avatar {
  border-radius: 100px !important;
