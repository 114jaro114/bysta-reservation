        ...
        <div class="title-photo position-relative">
          <div v-for="(image, index) in media" :key="'media' + index" class="d-flex child-flex container_images" cols="12" lg="4" md="4" sm="6" style="overflow: hidden">
            <v-card class="card_images" elevation="0" tile>
              <v-img style="cursor: pointer" class="zoom position-relative" :lazy-src="image.thumb" :src="image.thumb" height="200px" @click="openGallery(index)">
              ...
              <v-card-title v-else>
                <v-dialog v-model="dialog" width="500">
                  <template v-slot:activator="{ on: onDialog, attrs }">
                    ...
                    <div class="position-relative" v-else>
                      ...
                      <v-avatar color="primary" size="70">
                        <v-img v-for="(image, index) in media2" :key="'media2' + index" style="overflow: hidden; cursor: pointer" :lazy-src="image.thumb" :src="image.thumb" @click="openGallery2(index)">
                          <template v-slot:placeholder>
                            <v-row class="fill-height ma-0" align="center" justify="center">
                              <v-progress-circular indeterminate color="primary"></v-progress-circular>
                            </v-row>
                  ...
                  <v-card>
                    ...
                    <div>
                      <v-row class="m-0 p-3 justify-center canvas-profile-picture">
...
export default {
  ...
  data() {
    return {
      ...
      media: [{
        ...
        thumb: 'https://www.dropbox.com/s/fmgyq7woehxyt23/title_photo.png?raw=1',
        src: 'https://www.dropbox.com/s/fmgyq7woehxyt23/title_photo.png?raw=1',
        caption: '', // Optional
        srcset: 'https://www.dropbox.com/s/fmgyq7woehxyt23/title_photo.png?raw=1' // Optional for displaying responsive images
      }],
      media2: [{
        type: 'image', // Can be omitted for image
        thumb: '',
  ...
  updated() {
    if (this.$root.me.avatar) {
      this.media2[0].thumb = `${this.$root.envUrlNoApi}/storage/user-avatar/${this.$root.me.avatar}`
      this.media2[0].src = `${this.$root.envUrlNoApi}/storage/user-avatar/${this.$root.me.avatar}`
      this.media2[0].srcset = `${this.$root.envUrlNoApi}/storage/user-avatar/${this.$root.me.avatar}`
    }
    // this.checkIfUserIsMyFriend();
    // this.saveClicked()
